        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (Session["studID"] != null)
            {
                ...
                if (!IsPostBack)
                {
                    ...
                    conn.Open();
                    string sql = "SELECT c.CourseID, c.CourseName,CONVERT(varchar,'T')+CONVERT(varchar,t.TutorialNumber) AS 'Tutorial Number', t.ChapterName,  t.CompulsaryEasy+t.CompulsaryHard+t.CompulsaryMedium AS NoOfQuestion, k.Status as 'Completed' FROM Student s,Intake i, TutorialGroup g, Tutorial t, Course c, CourseAvailable v, TutorialCheck k WHERE s.IntakeID = i.IntakeID AND s.TutorialGroupID = g.TutorialGrpID AND g.TutorialGrpID = v.TutorialGrpID AND v.IntakeID = i.IntakeID AND c.CourseID = v.CourseID AND s.StudentID = k.StudentID AND k.TutorialID = t.TutorialID AND c.CourseID = t.CourseID AND v.Status = 1 AND t.Status = 1 AND s.StudentID = @studID AND k.Status = 1 GROUP BY s.StudentID, t.StartDate, t.ExpiryDate, t.TutorialID, t.TutorialNumber,t.ChapterName,c.CourseID,c.CourseName,t.CompulsaryEasy + t.CompulsaryHard + t.CompulsaryMedium,k.Status";
