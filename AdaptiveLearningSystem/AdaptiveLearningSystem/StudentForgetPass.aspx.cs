...
namespace AdaptiveLearningSystem
{
    public partial class StudentForgetPass : System.Web.UI.Page
    {
        ...
        {
            string email = "";
            Session["RecoverStudentID"] = txtStudentID.Text.Trim();
            SqlCommand cmd = new SqlCommand("prc_check_student_ID", conn);
            cmd.Parameters.AddWithValue("@StudentID", Session["RecoverStudentID"].ToString());
            cmd.CommandType = CommandType.StoredProcedure;
            conn.Open();
            SqlDataReader studentIDRead = cmd.ExecuteReader();
            if (!studentIDRead.HasRows)
            {
                lblError.Style.Add("display", "inherit");
                lblError.Text = "Student ID Not Exists.";
            }
            else
            {
                while (studentIDRead.Read())
                {
                     email = studentIDRead.GetString(0);
...
Thank You.";
        ...
        protected void btnChange_Click(object sender, EventArgs e)
        {
            ...
            {
                SqlCommand cmd = new SqlCommand("prc_chg_pass_by_student_id", conn);
                cmd.Parameters.AddWithValue("@StudentID", Session["RecoverStudentID"].ToString());
